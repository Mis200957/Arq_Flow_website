            ...
            <BarChart data={chartData} barSize={24}>
              ...
              <Bar dataKey="count" radius={[6, 6, 0, 0]}>
                {chartData.map((entry, i) => (
                  <Cell
                    key={i}
                    fill={entry.count === maxMessages ? "var(--accent)" : "rgba(27,27,30,0.3)"}
                  />
                ))}
              </Bar>
            </BarChart>
            ...
            {availableQuickActions.map((mod) => {
              ...
              return (
                <Link
                  key={mod.key}
                  href={mod.href}
                  className="flex items-center gap-3 p-3 rounded-xl hover:bg-[rgba(27,27,30,0.06)] transition-colors group"
                >
                  <div className="w-8 h-8 rounded-lg bg-[rgba(27,27,30,0.12)] flex items-center justify-center">
