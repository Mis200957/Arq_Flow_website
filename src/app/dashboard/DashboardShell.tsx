      ...
      >
            ...
            if (!mod.available) {
              return (
                ...
                >
                  ...
                  <span className="ms-auto shrink-0 text-[9px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded-md bg-[rgba(27,27,30,0.07)] text-muted">
                    {soonLabel}
                  </span>
                </div>
              );
            ...
            if (mod.locked) {
              return (
                <Link
                  ...
                  onClick={() => setSidebarOpen(false)}
                  title={upgradeLabel}
                  className="group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-muted/70 hover:bg-[rgba(27,27,30,0.06)] hover:text-app transition-all"
                >
                  <Icon className="w-4 h-4 shrink-0" />
                  <span className="truncate">{label}</span>
        ...
        <div className="p-3 border-t border-[var(--border)]">
          <div className="flex items-center gap-3 px-3 py-2">
            <div className="w-8 h-8 rounded-full bg-[rgba(27,27,30,0.2)] flex items-center justify-center text-accent font-bold text-sm shrink-0">
                      ...
                      {notifications.length === 0 ? (
                        ...
                        notifications.map((n) => (
                          <div
                            key={n.id}
                            className={cn(
                              "p-4 border-b border-[var(--border)] last:border-0 hover:bg-[rgba(27,27,30,0.04)] cursor-pointer",
                              !n.read && "bg-[rgba(27,27,30,0.06)]"
                            )}
                            onClick={() => {
